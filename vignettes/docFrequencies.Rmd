...
vignette: >
  %\VignetteIndexEntry{"The docFrequencies Object"}
...
# Introduction
***
The `docFrequencies` object can tell you a lot about the composition of a document.  It will give you a list of all of the words that appear in each document and a count of the number of times that the words appear.  It's a good starting point for analysis because you can get a sense of what the document is concerned with by looking at the most common words.  After looking at the most frequent words, you might be able to find a key word that you want to investigate with a `docConcordance` or a `docAssociation`.

To create a `docFrequencies` object, you will need a `docList` and a `docText` object.  From there, creating a `docFrequencies` object is simple: `df = frequencies(dt)`.  To see the frequencies of words in a given document, you can run: `df@raw[[N]]` where N is the number the document is in your `index`.
