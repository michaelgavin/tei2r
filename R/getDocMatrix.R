...
getDocMatrix = function(dt, type = "docTerm", wordLimit = 5000, modelSize = 500, context = 5, method = "raw") {
  dm = docMatrix()
  ...
  if (type == "docTerm") {
    ...
    dm@dtm = matrix(0, length(vocab), length(dt@text))
    ...
    for (i in 1:length(dt@text)) {
      if (class(wordLimit) == "NULL") {
        ...
        wordfreqs = sort(table(dt@text[i]), decreasing = T)[1:wordLimit]
        words = names(wordfreqs)
        freqs = as.numeric(wordfreqs)
        if (method == "raw") { dm@mat[words,i] = freqs }
        if (method == "proportional") { dm@mat[words,i] = freqs / sum(freqs)  }
  ...
  if (type == "wordContext") {
    ...
    keywords = names(sort(table(unlist(dt@text)), decreasing = T))[1:modelSize]
